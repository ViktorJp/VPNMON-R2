...
# VPNMON-R2 v1.3 (VPNMON-R2.SH) is an all-in-one simple script which compliments @JackYaz's VPNMGR program to maintain a
...
# -------------------------------------------------------------------------------------------------------------------------
Version="1.3"                                       # Current version of VPNMON-R2
...
vpnreset() {
  ...
  # Export NordVPN IPs via API into a txt file, and import them into Skynet
    ...
    then
      curl --silent --retry 3 "https://api.nordvpn.com/v1/servers?limit=16384" | jq --raw-output '.[] | select(.locations[].country.name == "'"$NordVPNCountry"'") | .station' > /jffs/scripts/NordVPN.txt
  ...
  # Start the selected VPN Client
    ...
    # Sync active VPN Slot with YazFi guest network(s)
    if [[ $SyncYazFi -eq 1 ]]
    then
      echo -e "${CCyan}Updating YazFi Guest Network(s) with current VPN Slot...\n${CClear}"
      echo ""
      YAZFI_CONFIG_PATH="/jffs/addons/YazFi.d/config"

      if [ ! -f $YAZFI_CONFIG_PATH ]
        then
          echo -e "\n${CCyan}Error: YazFi config was not located or YazFi is not installed. Unable to Proceed.\n${CClear}"
          echo -e "$(date) - VPNMON-R2 - *** ERROR: YazFi config was not located or YazFi is not installed!" >> $LOGFILE
        else
          if [[ $YF24GN1 -eq 1 ]]
          then
            sed "s/^wl01_VPNCLIENTNUMBER=.*/wl01_VPNCLIENTNUMBER=$option/" "$YAZFI_CONFIG_PATH" > /jffs/scripts/temp.x
            mv /jffs/scripts/temp.x "$YAZFI_CONFIG_PATH"
          fi

          if [[ $YF24GN2 -eq 1 ]]
          then
            sed "s/^wl02_VPNCLIENTNUMBER=.*/wl02_VPNCLIENTNUMBER=$option/" "$YAZFI_CONFIG_PATH" > /jffs/scripts/temp.x
            mv /jffs/scripts/temp.x "$YAZFI_CONFIG_PATH"
          fi

          if [[ $YF24GN3 -eq 1 ]]
          then
            sed "s/^wl03_VPNCLIENTNUMBER=.*/wl03_VPNCLIENTNUMBER=$option/" "$YAZFI_CONFIG_PATH" > /jffs/scripts/temp.x
            mv /jffs/scripts/temp.x "$YAZFI_CONFIG_PATH"
          fi

          if [[ $YF5GN1 -eq 1 ]]
          then
            sed "s/^wl11_VPNCLIENTNUMBER=.*/wl11_VPNCLIENTNUMBER=$option/" "$YAZFI_CONFIG_PATH" > /jffs/scripts/temp.x
            mv /jffs/scripts/temp.x "$YAZFI_CONFIG_PATH"
          fi

          if [[ $YF5GN2 -eq 1 ]]
          then
            sed "s/^wl12_VPNCLIENTNUMBER=.*/wl12_VPNCLIENTNUMBER=$option/" "$YAZFI_CONFIG_PATH" > /jffs/scripts/temp.x
            mv /jffs/scripts/temp.x "$YAZFI_CONFIG_PATH"
          fi

          if [[ $YF5GN3 -eq 1 ]]
          then
            sed "s/^wl13_VPNCLIENTNUMBER=.*/wl13_VPNCLIENTNUMBER=$option/" "$YAZFI_CONFIG_PATH" > /jffs/scripts/temp.x
            mv /jffs/scripts/temp.x "$YAZFI_CONFIG_PATH"
          fi

          if [[ $YF52GN1 -eq 1 ]]
          then
            sed "s/^wl21_VPNCLIENTNUMBER=.*/wl21_VPNCLIENTNUMBER=$option/" "$YAZFI_CONFIG_PATH" > /jffs/scripts/temp.x
            mv /jffs/scripts/temp.x "$YAZFI_CONFIG_PATH"
          fi

          if [[ $YF52GN2 -eq 1 ]]
          then
            sed "s/^wl22_VPNCLIENTNUMBER=.*/wl22_VPNCLIENTNUMBER=$option/" "$YAZFI_CONFIG_PATH" > /jffs/scripts/temp.x
            mv /jffs/scripts/temp.x "$YAZFI_CONFIG_PATH"
          fi

          if [[ $YF52GN3 -eq 1 ]]
          then
            sed "s/^wl23_VPNCLIENTNUMBER=.*/wl23_VPNCLIENTNUMBER=$option/" "$YAZFI_CONFIG_PATH" > /jffs/scripts/temp.x
            mv /jffs/scripts/temp.x "$YAZFI_CONFIG_PATH"
          fi
  ...
  if [ "$1" == "-config" ]
    ...
    if [ -f $CFGPATH ] #Making sure file exists before proceeding
      then
        ...
        echo -e "This process is the same for a new installation, or if you wanted to"
        echo -e "configure VPNMON-R2 with different options. Upon completion, you have"
        echo -e "the option to overwrite an existing config file. Would you like to"
        echo -e "continue?${CClear}"
        ...
        if [ "$UseNordVPN" == "1" ]; then
          ...
          echo -e "United States). NOTE: Country names must be spelled correctly as below!${CCyan}"
          echo "Valid country names as follows: Albania, Argentina, Australia, Austria,"
          echo "Belgium, Bosnia and Herzegovina, Brazil, Bulgaria, Canada, Chile,"
          echo "Costa Rica, Croatia, Cyprus, Czech Republic, Denmark, Estonia, Finland,"
          echo "France, Georgia, Germany, Greece, Hong Kong, Hungary, Iceland, India,"
          echo "Indonesia, Ireland, Israel, Italy, Japan, Latvia, Lithuania, Luxembourg,"
          echo "Malaysia, Mexico, Moldova, Netherlands, New Zealand, North Macedonia,"
          echo "Norway, Poland, Portugal, Romania, Serbia, Singapore, Slovakia, Slovenia,"
          echo "South Africa, South Korea, Spain, Sweden, Switzerland, Taiwan, Thailand,"
          echo "Turkey, Ukraine, United Arab Emirates, United Kingdom, United States,"
          echo -e "Vietnam.${CClear}"
          read -p 'NordVPN Country: ' NordVPNCountry
          echo ""
          echo -e "${CCyan}5c. At what VPN server load would you like to reconnect to a different"
          echo -e "NordVPN Server? ${CYellow}(Default = 50)${CClear}"
          read -p 'Server Load Threshold: ' NordVPNLoadReset
          echo ""
          echo -e "${CCyan}5d. Would you like to whitelist NordVPN servers in the Skynet Firewall?"
          echo -e "${CYellow}(Default = No)${CClear}"
          if promptyn "(Yes/No): "; then
            UpdateSkynet=1
          else
        ...
        echo ""
        echo -e "${CCyan}6. Would you like to reset your VPN connection to a random VPN client daily?"
        echo -e "${CYellow}(Default = Yes)${CClear}"
        if promptyn "(Yes/No): "; then
          ResetOption=1
          echo ""
          echo -e "${CCyan}6a. What time would you like to reset your connection? ${CYellow}(Default = 01:00)${CClear}"
        ...
        echo ""
        echo -e "${CCyan}7. How many VPN client slots do you have properly configured? ${CYellow}(Default = 5)${CClear}"
        read -p 'VPN Clients: ' N
        echo ""
        echo -e "${CCyan}8. Would you like to show near-realtime VPN stats on the UI? ${CYellow}(Default = No)${CClear}"
        if promptyn "(Yes/No): "; then
          SHOWSTATS=1
        else
          SHOWSTATS=0
        ...
        echo ""
        echo -e "${CCyan}9. Would you like to sync the active VPN slot with YazFi? ${CYellow}(Default = No)${CClear}"
        if promptyn "(Yes/No): "; then
          SyncYazFi=1
        else
          SyncYazFi=0
        ...
        if [ "$SyncYazFi" == "1" ]; then
          echo ""
          echo -e "${CCyan}9a. Please indicate which of your YazFi guest network slots you want to sync with"
          echo -e "${CCyan}the active VPN slot?"
          echo ""
        ...
        logo
        echo -e "Configuration of VPNMON-R2 is complete.  Would you like to save this config?${CClear}"
        if promptyn "(Yes/No): "; then
          { echo 'TRIES='$TRIES
