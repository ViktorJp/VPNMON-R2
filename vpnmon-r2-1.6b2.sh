...
# VPNMON-R2 v1.6 (VPNMON-R2.SH) is an all-in-one shell script which compliments @JackYaz's VPNMGR program to maintain a
...
# -------------------------------------------------------------------------------------------------------------------------
Version="1.6b2"                                     # Current version of VPNMON-R2
    ...
    if [ -f $CFGPATH ] #Making sure file exists before proceeding
      then
        ...
        if [ "$UseNordVPN" == "1" ]; then
          ...
          echo ""
          echo -e "${CCyan}6. Is SurfShark your default VPN Provider? ${CYellow}(Default = No)${CClear}"
          if promptyNo; then
            UseSurfShark=1
          else
            UseSurfShark=0
...
while true; do
  ...
  if [ $SHOWSTATS == "1" ]
    ...
    if [ -z "$VPNLOAD" ]; then VPNLOAD=0; fi # On that rare occasion where it's unable to get the NordVPN/SurfShark load, assign 0

    # Display some of the NordVPN or SurfShark specific stats
    if [ $NordVPNSuperRandom -eq 1 ] || [ $UseNordVPN -eq 1 ] || [ $SurfSharkSuperRandom -eq 1 ] || [ $UseSurfShark -eq 1 ]
      then
        ...
        # Display the high/low ping times, and for non-NordVPN/SurfShark customers, whether Skynet update is enabled.
