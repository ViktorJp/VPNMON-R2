...
# VPNMON-R2 v2.45b2 (VPNMON-R2.SH) is an all-in-one script that is optimized for NordVPN, SurfShark VPN and Perfect Privacy
...
# -------------------------------------------------------------------------------------------------------------------------
Version="2.45b2"                                    # Current version of VPNMON-R2
      ...
      then
        ...
        while [ $svrcount -ne 15 ]
          do
            ...
            if [ $LINES -ge 1 ]; then
              #curl --silent --retry 3 https://airvpn.org/api/status/ | jq --raw-output '.servers[] | select(.country_name=="'"$AirVPNRandomCountry"'").ip_v4_in2' >> /jffs/scripts/AirVPN.txt
              #curl --silent --retry 3 https://airvpn.org/api/status/ | jq --raw-output '.servers[] | select(.country_name=="'"$AirVPNRandomCountry"'").ip_v4_in3' >> /jffs/scripts/AirVPN.txt
        ...
        then
          ...
          while [ $i -ne $N ] #Assign SuperRandom IPs/Descriptions to VPN Slots 1-N
            do
              ...
              #RNDVPNHOST=$(curl --silent --retry 3 https://api.nordvpn.com/v1/servers?limit=16384 | jq --raw-output '.[] | select(.station == "'"$RNDVPNIP"'") | .hostname' | cut -d '.' -f1)
      ...
      if [ $UseNordVPN == "1" ]; then
        ...
        while [ $i -ne $N ] #Assign Recommended IPs/Descriptions to VPN Slots 1-N
          do
            ...
            #RECVPNHOST=$(curl --silent --retry 3 https://api.nordvpn.com/v1/servers?limit=16384 | jq --raw-output '.[] | select(.station == "'"$RECVPNIP"'") | .hostname' | cut -d '.' -f1)
        ...
        RECVPNIP1=$(ping -q -c1 -n $AirTemp1 | head -n1 | sed "s/.*(\([0-9]*\.[0-9]*\.[0-9]*\.[0-9]*\)).*/\1/g") > /dev/null 2>&1 #2>/dev/null
        RECVPNIP2=$(ping -q -c1 -n $AirTemp2 | head -n1 | sed "s/.*(\([0-9]*\.[0-9]*\.[0-9]*\.[0-9]*\)).*/\1/g") > /dev/null 2>&1 #2>/dev/null
...
while true; do
  ...
  if [ -f /jffs/addons/killmon.d/killmon.cfg ]; then
    ...
    SUPPRESSVPNMONR2=$(cat /jffs/addons/killmon.d/killmon.cfg | sed -n '6p' | cut -d '"' -f2) 2>&1
  ...
  if [ "$SHOWSTATS" == "1" ]
    ...
    if [ $AirVPNSuperRandom -eq 1 ] || [ $UseAirVPN -eq 1 ]
      then
        ...
        #AirVPNLOAD1=$(curl --silent --retry 3 https://airvpn.org/api/status/ | jq --raw-output '.servers[] | select(.ip_v4_in1=="'"$VPNIP"'").currentload' 2>&1)
        #AirVPNLOAD2=$(curl --silent --retry 3 https://airvpn.org/api/status/ | jq --raw-output '.servers[] | select(.ip_v4_in2=="'"$VPNIP"'").currentload' 2>&1)
