...
# VPNMON-R2 v2.55b3 (VPNMON-R2.SH) is an all-in-one script that is optimized for NordVPN, SurfShark VPN and Perfect Privacy
...
# -------------------------------------------------------------------------------------------------------------------------
Version="2.55b3"                                    # Current version of VPNMON-R2
    ...
    # Wait for confirmation that all VPN client slots are at 0 (disconnected)
      ...
      # Optionally sync active VPN Slot with YazFi guest network(s)
        ...
        if [ $UpdateUnbound -eq 1 ]; then
          ...
          sh /jffs/addons/unbound/unbound_manager.sh vpn=$LOWEST >/dev/null 2>&1
        fi
...
vpnreset() {
  ...
  # Call VPNMGR functions to refresh server lists and save their results to the VPN client configs
    ...
    if [ $UpdateUnbound -eq 1 ]; then
      ...
      sh /jffs/addons/unbound/unbound_manager.sh vpn=$option >/dev/null 2>&1
    fi
          ...
          while true; do
            ...
            # Execute chosen selections
                case "$ConfigSelection" in
                  ...
                  17) # -----------------------------------------------------------------------------------------
                     echo ""
                     echo -e "${CCyan} 17. Would you like to update Unbound to use your current VPN slot?"
                     echo -e "${CCyan} After a VPN reset, VPNMON-R2 will force Unbound to use the current"
                     echo -e "${CCyan} VPN Slot and force DNS lookups over port 53 through your VPN Tunnel"
                     echo -e "${CCyan} instead of over the open Internet to the DNS Root Servers. Please"
                     echo -e "${CCyan} note that this functionality requires you to already have the Unbound"
                     echo -e "${CCyan} utility installed (via AMTM), and must be functional. Enabling this"
                     echo -e "${CCyan} functionality will also add a vpn=disable Unbound command to your"
                     echo -e "${CCyan} routers' POST-MOUNT file as a fail-safe to bypass a downed VPN"
                     echo -e "${CCyan} tunnel should your router require a reboot."
                     echo -e "${CYellow} (No=0, Yes=1) (Default = 0)${CClear}"
                     ...
                     if [ "$UpdateUnbound" == "0" ]; then
                       ...
                       /jffs/addons/unbound/unbound_manager.sh vpn=disable >/dev/null 2>&1
