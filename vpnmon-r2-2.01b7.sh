...
# VPNMON-R2 v2.01b7 (VPNMON-R2.SH) is an all-in-one script that is optimized for NordVPN, SurfShark VPN and Perfect Privacy
...
# -------------------------------------------------------------------------------------------------------------------------
Version="2.01b7"                                    # Current version of VPNMON-R2
...
checkwan () {
  ...
  while true; do
    ...
    then
      ...
      if ($timeoutcmd$timeoutlng nc -w1 $testssl 443 && echo | $timeoutcmd$timeoutlng openssl s_client -connect $testssl:443 2>&1 |awk 'handshake && $1 == "Verification" { if ($2=="OK") exit; exit 1 } $1 $2 == "SSLhandshake" { handshake = 1 }')
        then
          if [ "$1" == "Loop" ]
          then
