...
# VPNMON-R2 v2.27b5 (VPNMON-R2.SH) is an all-in-one script that is optimized for NordVPN, SurfShark VPN and Perfect Privacy
...
# -------------------------------------------------------------------------------------------------------------------------
Version="2.27b5"                                    # Current version of VPNMON-R2
...
#{              # uncomment/comment to enable/disable debug mode
  ...
  if [ "$1" == "-pause" ]
    then
      ...
      if [ -z $LASTSLOT ]; then LASTSLOT=1; fi
  ...
  if [ "$1" == "-status" ]
    then
      ...
      STATE=$(cat $APPSTATUS | sed -n '1p')
      LASTSLOT=$(cat $APPSTATUS | sed -n '2p')
