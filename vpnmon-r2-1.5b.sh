...
# VPNMON-R2 v1.5alpha (VPNMON-R2.SH) is an all-in-one simple script which compliments @JackYaz's VPNMGR program to maintain a
...
# -------------------------------------------------------------------------------------------------------------------------
Version="1.5b"                                      # Current version of VPNMON-R2
        ...
        while [ $CNT -lt $TRIES ]; do
          ...
          if [ $RC -eq 0 ] && [ $IC -eq 0 ];then
            ...
            if [ $VPNIP == "Unassigned" ];then
              ...
              if [ $VPNCITY == "Undefined" ]; then VPNCITY=$ICANHAZIP; fi # On that rare occasion that a city comes up undefined
...
# -------------------------------------------------------------------------------------------------------------------------
  ...
  if [ "$1" == "-h" ] || [ "$1" == "-help" ] || [ "$1" == "-config" ] || [ "$1" == "-monitor" ] || [ "$1" == "-log" ] || [ "$1" == "-update" ] || [ "$1" == "-uninstall" ]
    then
      clear
    else
      clear
