...
# VPNMON-R2 v2.25b3 (VPNMON-R2.SH) is an all-in-one script that is optimized for NordVPN, SurfShark VPN and Perfect Privacy
...
# -------------------------------------------------------------------------------------------------------------------------
Version="2.25b3"                                    # Current version of VPNMON-R2
      ...
      then
        ...
        while [ $svrcount -ne 60 ]
          do
            ...
            NORDLINES="$(eval $NORDLINES)"; if echo $NORDLINES | grep -qoE '\berror.*\b'; then printf "${CRed}\r [Error Occurred]"; sleep 1; fi
      ...
      then
        ...
        while [ $loadcount -ne 60 ]
          do
            ...
            VPNLOAD="$(eval $VPNLOAD)"; if echo $VPNLOAD | grep -qoE '\berror.*\b'; then VPNLOAD=0; printf "${CRed}\r [API Error Occurred... retrying $loadcount/60]           "; sleep 1; fi
