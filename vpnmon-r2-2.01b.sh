...
# VPNMON-R2 v2.01b (VPNMON-R2.SH) is an all-in-one script that is optimized for NordVPN, SurfShark VPN and Perfect Privacy
...
# -------------------------------------------------------------------------------------------------------------------------
Version="2.01b"                                     # Current version of VPNMON-R2
  ...
  if [ $WANIF == "0" ]; then
    if [ "$($timeoutcmd$timeoutsec nvram get wan0_state_t)" -eq 2 ]
      then
        ...
        WAN0PING=$(ping -I $WAN0IFNAME -c 1 $PINGHOST | awk -F'time=| ms' 'NF==3{print $(NF-1)}' | sort -rn)
        ...
        #WANCITY="Your City"
        echo -e "${CGreen} ==WAN$WAN0IF $WAN0IFNAME Active | ||${CWhite}${InvGreen} $WAN0PING ms ${CClear}${CGreen}|| | ${CClear}${CYellow}Exit: ${InvBlue}$WAN0CITY${CClear}"

      else
        echo -e "${CClear} - WAN$WAN0IF Port Inactive"
    fi
  fi
  ...
  if [ $WANIF == "1" ]; then
    if [ "$($timeoutcmd$timeoutsec nvram get wan1_state_t)" -eq 2 ]
      then
        ...
        # Ping through the WAN interface
        WAN1PING=$(ping -I $WAN1IFNAME -c 1 $PINGHOST | awk -F'time=| ms' 'NF==3{print $(NF-1)}' | sort -rn)
        ...
        #WANCITY="Your City"
        echo -e "${CGreen} ==WAN$WAN1IF $WAN1IFNAME Active | ||${CWhite}${InvGreen} $WAN1PING ms ${CClear}${CGreen}|| | ${CClear}${CYellow}Exit: ${InvBlue}$WAN1CITY${CClear}"

      else
        echo -e "${CClear} - WAN$WAN1IF Port Inactive"
    fi
  fi
}
