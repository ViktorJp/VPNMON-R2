...
# VPNMON-R2 v2.36b2 (VPNMON-R2.SH) is an all-in-one script that is optimized for NordVPN, SurfShark VPN and Perfect Privacy
...
# -------------------------------------------------------------------------------------------------------------------------
Version="2.36b2"                                    # Current version of VPNMON-R2
...
lockcheck () {
  ...
  if [ -f $APPSTATUS ]; then
    ...
    while [ $STATE == "STOPPED" ]; do
      ...
      echo ""
      echo -e "${CGreen} VPNMON-R2 is currently operating in a STOPPED state. All VPN"
  ...
  if [ "$StopLockFound" == "1" ]; then
    ...
    SPIN=5
    echo ""
    printf "${CGreen}\r [Attempting restart of last known good VPN$LASTSLOT client and restarting UI]...${CClear}\n"
    ...
    while [ $ResVPNState -ne 2 ]; do
      ...
      if [ $i -eq 60 ]; then
        echo ""
        printf "${CGreen}\r [Unable to restart of last known good VPN$LASTSLOT client...Retrying]...            ${CClear}\n"
      ...
      if [ $i -eq 120 ]; then
        echo ""
        printf "${CGreen}\r [Unable to restart of last known good VPN$LASTSLOT client...VPN Reset Initiating]...${CClear}\n"
        service stop_vpnclient$LASTSLOT >/dev/null 2>&1
        sleep 5
        break
      fi
...
vupdate () {
  ...
  if [ "$Version" == "$DLVersion" ]
    then
      ...
      echo -e "${CCyan} You are NOT on the latest version! Would you like to update?${CClear}"
      if promptyn " (y/n): "; then
        echo ""
        echo -e "${CCyan} Downloading VPNMON-R2 ${CYellow}v$DLVersion${CClear}"
        curl --silent --retry 3 "https://raw.githubusercontent.com/ViktorJp/VPNMON-R2/master/vpnmon-r2-$DLVersion.sh" -o "/jffs/scripts/vpnmon-r2.sh" && chmod a+rx "/jffs/scripts/vpnmon-r2.sh"
...
#{              # uncomment/comment to enable/disable debug mode
  ...
  if [ "$1" == "-h" ] || [ "$1" == "-help" ] || [ "$1" == "-config" ] || [ "$1" == "-monitor" ] || [ "$1" == "-log" ] || [ "$1" == "-update" ] || [ "$1" == "-setup" ] || [ "$1" == "-uninstall" ] || [ "$1" == "-screen" ] || [ "$1" == "-reset" ] || [ "$1" == "-pause" ] || [ "$1" == "-stop" ] || [ "$1" == "-resume" ] || [ "$1" == "-status" ]
    then
      clear
    else
      clear
