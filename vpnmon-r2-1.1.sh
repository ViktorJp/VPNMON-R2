...
#VPNMON-R2 v1.1 (VPNMON-R2.SH) is an all-in-one simple script which compliments @JackYaz's VPNMGR program to maintain a
...
# for any current automations you may have in place.  To easily view the log file, enter: "vpnmon-r2.sh -log".
#
# -------------------------------------------------------------------------------------------------------------------------
# System Variables (Do not change beyond this point or this may change the programs ability to function correctly)
# -------------------------------------------------------------------------------------------------------------------------
Version="1.1"                                       # Current version of VPNMON-R2
...
# -------------------------------------------------------------------------------------------------------------------------
  ...
  if [ "$1" == "-h" ] || [ "$1" == "-help" ] || [ "$1" == "-config" ] || [ "$1" == "-monitor" ] || [ "$1" == "-log" ]
    then
      clear
    else
      clear
  ...
  if [ "$1" == "-h" ] || [ "$1" == "-help" ]
    ...
    clear
    echo ""
    echo "VPNMON-R2 $Version Commandline Option Usage:"
    echo ""
  ...
  if [ "$1" == "-config" ]
    ...
    if [ -f $CFGPATH ] #Making sure file exists before proceeding
      then
        ...
        echo ""
        echo -e "VPNMON-R2 $Version Configuration Utility${CClear}"
        echo ""
        ...
        if promptyn "(Yes/No): "; then
          { echo 'TRIES='$TRIES
            ...
            echo ""
            echo -e "${CRed}Discarding changes and exiting setup.${CClear}"
            echo ""
            kill 0
        fi
        echo ""
...
while true; do
  ...
  if [[ $NordVPNSuperRandom -eq 1 ]] || [[ $UseNordVPN -eq 1 ]]
    then
      ...
      VPNLOAD=$(curl --silent "https://api.nordvpn.com/v1/servers?limit=16354" | jq '.[] | select(.station == "'"$VPNIP"'") | .load')
      printf "\r"
