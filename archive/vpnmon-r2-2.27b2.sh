...
# VPNMON-R2 v2.27b2 (VPNMON-R2.SH) is an all-in-one script that is optimized for NordVPN, SurfShark VPN and Perfect Privacy
...
# -------------------------------------------------------------------------------------------------------------------------
Version="2.27b2"                                    # Current version of VPNMON-R2
...
vpnreset() {
  ...
  # Call VPNMGR functions to refresh server lists and save their results to the VPN client configs
    ...
    # Start the selected VPN Client
      ...
      printf "${CGreen}\r                                                               "
      printf "${CGreen}\r [VPN$option Client ON]                                              "
...
#{              # uncomment/comment to enable/disable debug mode
  ...
  if [ "$1" == "-pause" ]
    then
      clear
      echo -e "PAUSED" > $APPSTATUS
      echo -e "${CGreen} VPNMON-R2 is entering a PAUSED state...${CClear}"
      echo -e "${CGreen} Use 'vpnmon-r2 -resume' to return to normal operations.${CClear}"
  ...
  if [ "$1" == "-resume" ]
    then
      clear
      echo -e "NORMAL" > $APPSTATUS
      echo -e "${CGreen} VPNMON-R2 is returning to a NORMAL operations state...${CClear}"
  ...
  if [ "$1" == "-status" ]
    then
      clear
      STATE=$(cat $APPSTATUS) 2>&1
      if [ -z $STATE ]; then STATE="UNKNOWN"; fi
      echo -e "${CGreen} VPNMON-R2 is currently operating in a $STATE state...${CClear}"
      echo -e "$(date) - VPNMON-R2 ----------> STATUS: $STATE" >> $LOGFILE
      echo ""
      exit 0
  fi
