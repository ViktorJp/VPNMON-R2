...
# VPNMON-R2 v2.25b1 (VPNMON-R2.SH) is an all-in-one script that is optimized for NordVPN, SurfShark VPN and Perfect Privacy
...
# -------------------------------------------------------------------------------------------------------------------------
Version="2.25b1"                                    # Current version of VPNMON-R2
...
vpnreset() {
  ...
  # Call VPNMGR functions to refresh server lists and save their results to the VPN client configs
    if [ $UpdateVPNMGR -eq 1 ]; then
      ...
      if [ $UseNordVPN -eq 1 ]; then
        # Translate IP addresses back to hostnames for VPNMGR
        i=0
        while [ $i -ne $N ]
          do
            i=$(($i+1))
            VPNMGRVPNIP=$($timeoutcmd$timeoutsec nvram get vpn_client"$i"_addr)
            VPNMGRVPNHOST=$(curl --silent --retry 3 "https://api.nordvpn.com/v1/servers?limit=16384" | jq --raw-output '.[] | select(.station == "'"$VPNMGRVPNIP"'") | .hostname')
            nvram set vpn_client"$i"_addr="$VPNMGRVPNHOST"
        done
      fi
