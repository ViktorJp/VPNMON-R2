...
# VPNMON-R2 v1.5 (VPNMON-R2.SH) is an all-in-one simple script which compliments @JackYaz's VPNMGR program to maintain a
...
# -------------------------------------------------------------------------------------------------------------------------
Version="1.5"                                       # Current version of VPNMON-R2
      ...
      then
        ...
        while [ $i -ne $N ] #Assign SuperRandom IPs/Descriptions to VPN Slots 1-N
          do
            ...
            #RNDVPNCITY="$(eval $RNDVPNCITY)"; if echo $RNDVPNCITY | grep -qE '\b(.*'error'.*:.*True.*|Undefined)\b'; then RNDVPNCITY="$RNDVPNIP"; fi
        ...
        while [ $CNT -lt $TRIES ]; do # Loop through number of tries
          ...
          if [ $RC -eq 0 ] && [ $IC -eq 0 ];then  # If both ping/curl come back successful, then proceed
            ...
            if [ $VPNIP == "Unassigned" ];then # The first time through, use API lookup to get exit VPN city and display
              ...
              #VPNCITY="$(eval $VPNCITY)"; if echo $VPNCITY | grep -qE '\b(.*'error'.*:.*True.*|Undefined)\b'; then VPNCITY="$ICANHAZIP"; fi
  ...
  if [ "$1" == "-config" ]
    ...
    if [ -f $CFGPATH ] #Making sure file exists before proceeding
      then
        ...
        echo ""
        echo -e "${CCyan}9. Would you like to delay start-up of VPNMON-R2 in order to provide"
        echo -e "${CCyan}more stability among other competing start-up scripts during a reboot?"
        ...
        echo ""
        echo -e "${CCyan}10. Would you like to sync the active VPN slot with YazFi?"
        echo -e "${CYellow}(Default = No)${CClear}"
        if promptyNo; then
          SyncYazFi=1
        else
