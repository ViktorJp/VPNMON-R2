...
# VPNMON-R2 v2.12b1 (VPNMON-R2.SH) is an all-in-one script that is optimized for NordVPN, SurfShark VPN and Perfect Privacy
...
# -------------------------------------------------------------------------------------------------------------------------
Version="2.12b1"                                    # Current version of VPNMON-R2
  ...
  then
    while [ $CNT -lt $TRIES ]; do # Loop through number of tries
      ...
      if [ $RC -eq 0 ] && [ $IC -eq 0 ];then  # If both ping/curl come back successful, then proceed
        ...
        echo -e "${CGreen} ==VPN$1 Tunnel Active | ||${CWhite}${InvGreen} $AVGPING ms ${CClear}${CGreen}|| | ${CClear}${CGreen}Exit: ${CWhite}${InvDkGray}$VPNCITY${CClear}"
        CURRCLNT=$1
        break
      else
    ...
    if [ "$($timeoutcmd$timeoutsec nvram get wan0_state_t)" -eq 2 ]
      then
        ...
        if [ $WAN0PING == "DW-FO" ]; then
          echo -e "${CGreen} ==WAN0 $WAN0IFNAME Active | ||${CWhite}${InvGreen} FAILOVER ${CClear}${CGreen}|| | ${CClear}${CGreen}Exit: ${CWhite}${InvDkGray}$WAN0CITY${CClear}"
        else
          echo -e "${CGreen} ==WAN0 $WAN0IFNAME Active | ||${CWhite}${InvGreen} $WAN0PING ms ${CClear}${CGreen}|| | ${CClear}${CGreen}Exit: ${CWhite}${InvDkGray}$WAN0CITY${CClear}"
        fi
    ...
    if [ "$($timeoutcmd$timeoutsec nvram get wan1_state_t)" -eq 2 ]
      then
        ...
        if [ $WAN1PING == "DW-FO" ]; then
          echo -e "${CGreen} ==WAN1 $WAN1IFNAME Active | ||${CWhite}${InvGreen} FAILOVER ${CClear}${CGreen}|| | ${CClear}${CGreen}Exit: ${CWhite}${InvDkGray}$WAN1CITY${CClear}"
        else
          echo -e "${CGreen} ==WAN1 $WAN1IFNAME Active | ||${CWhite}${InvGreen} $WAN1PING ms ${CClear}${CGreen}|| | ${CClear}${CGreen}Exit: ${CWhite}${InvDkGray}$WAN1CITY${CClear}"
        fi
...
while true; do
  ...
  echo -e "${CCyan} $(date)${CGreen} $dashes ${CGreen}Last Reset: ${CWhite}${InvDkGray}$LASTVPNRESET${CClear}"
  ...
  echo -e "${CCyan} VPN State 1:$state1 2:$state2 3:$state3 4:$state4 5:$state5${CClear}${CGreen} ---- ${CGreen}Public VPN IP: ${CWhite}${InvDkGray}$ICANHAZIP${CClear}"

  if [ $ResetOption -eq 1 ]
    then
      echo -e "${CCyan} WAN State 0:$wstate0 1:$wstate1${CGreen} ------------------ ${CGreen}Sched Reset: ${CWhite}${InvDkGray}$ConvDailyResetTime${CClear}${CYellow} / ${CWhite}${InvDkGray}$INTERVAL Sec${CClear}"
    else
      echo -e "${CCyan} WAN State 0:$wstate0 1:$wstate1${CGreen} --------------------- ${CGreen}Interval: ${CWhite}${InvDkGray}$INTERVAL Sec${CClear}"
  fi
  ...
  if [ "$SHOWSTATS" == "1" ]
    ...
    if [ $NordVPNSuperRandom -eq 1 ] || [ $UseNordVPN -eq 1 ] || [ $SurfSharkSuperRandom -eq 1 ] || [ $UseSurfShark -eq 1 ] || [ $PPSuperRandom -eq 1 ] || [ $UsePP -eq 1 ]
      then
        echo -e "${CGreen} Ping Lo:${CWhite}${InvGreen}$PINGLOW${CClear}${CGreen} Hi:${CWhite}${InvRed}$PINGHIGH${CClear}${CGreen} ms | Load: ${CWhite}${InvDkGray} $VPNLOAD% ${CClear}${CGreen} | Cfg: ${CWhite}${InvDkGray}$RANDOMMETHOD${CClear}"
        ...
        then
          echo -e "${CGreen} Ping Lo:${CWhite}${InvGreen}$PINGLOW${CClear}${CGreen} Hi:${CWhite}${InvRed}$PINGHIGH${CClear}${CGreen} ms | Cfg: ${CWhite}${InvDkGray}$RANDOMMETHOD${CClear}"
        else
          echo -e "${CGreen} Ping Lo:${CWhite}${InvGreen}$PINGLOW${CClear}${CGreen} Hi:${CWhite}${InvRed}$PINGHIGH${CClear}${CGreen} ms | Skynet: ${CWhite}${InvDkGray}[Y]${CClear}${CGreen} | Cfg: ${CWhite}${InvDkGray}$RANDOMMETHOD${CClear}"
    fi
    ...
    echo -e "${CGreen} Proto: ${CWhite}${InvDkGray}$vpnproto${CClear}${CGreen} | Port: ${CWhite}${InvDkGray}$vpnport${CClear}${CGreen} | Crypto: ${CWhite}${InvDkGray}$vpncrypto${CClear}${CGreen} | AuthDigest: ${CWhite}${InvDkGray}$vpndigest${CClear}"
    ...
    if [ "$oldrxbytes" == "0" ] || [ "$oldtxbytes" == "0" ]
      then
        ...
        echo -e "${CYellow} [Gathering VPN RX and TX Stats]... ${CGreen}| Ttl RX:${CWhite}${InvDkGray}$txgbytes GB${CClear} ${CGreen}TX:${CWhite}${InvDkGray}$rxgbytes GB${CClear}"
      else
        # Display current avg rx/tx rates and total rx/tx bytes for active VPN tunnel.
        echo -e "${CGreen} Avg RX:${CWhite}${InvDkGray}$txmbrate Mbps${CClear}${CGreen} TX:${CWhite}${InvDkGray}$rxmbrate Mbps${CClear}${CGreen} | Ttl RX:${CWhite}${InvDkGray}$txgbytes GB${CClear} ${CGreen}TX:${CWhite}${InvDkGray}$rxgbytes GB${CClear}"
    fi
