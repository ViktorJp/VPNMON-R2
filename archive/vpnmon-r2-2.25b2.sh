...
# VPNMON-R2 v2.25b2 (VPNMON-R2.SH) is an all-in-one script that is optimized for NordVPN, SurfShark VPN and Perfect Privacy
...
# -------------------------------------------------------------------------------------------------------------------------
Version="2.25b2"                                    # Current version of VPNMON-R2
  ...
  # Export NordVPN/PerfectPrivacy IPs via API into a txt file, and import them into Skynet
    ...
    then
      ...
      then
        ...
        while [ $svrcount -ne 60 ]
          do
            ...
            if [ $svrcount -eq 60 ]; then
              echo -e "\n${CRed}Error: Unable to reach NordVPN API! Check NordVPN service or config's Country Name.\n${CClear}"
        ...
        then
          echo -e "\n${CRed}Error: NordVPN.txt VPN Server list is blank! Skipping import into Skynet Firewall.\n${CClear}"
      ...
      then
        ...
        while [ $svrcount -ne 60 ]
          do
            ...
            if [ $svrcount -eq 60 ]; then
              echo -e "\n${CRed}Error: Unable to reach Perfect Privacy API! Check PP service or config's Country Name.\n${CClear}"
  ...
  # Randomly select VPN Client slots against entire field of available NordVPN server IPs for selected country
    ...
    then
      ...
      then
        ...
        if [ $LINES -eq 0 ] # If there are no lines, error out
        then
          echo -e "\n${CRed}Error: NordVPN.txt VPN Server list is blank! Skipping SuperRandom assignment process.\n${CClear}"
    ...
    then
      ...
      then
        ...
        if [ $LINES -eq 0 ] #If there are no lines, error out
        then
          echo -e "\n${CRed}Error: surfshark.txt VPN Serverlist is blank! Skipping SuperRandom assignment process.\n${CClear}"
      ...
      then
        ...
        while [ $svrcount -ne 60 ]
          do
            ...
            if [ $svrcount -eq 60 ]; then
              echo -e "\n${CRed}Error: Unable to reach Perfect Privacy API! Check Perfect Privacy service or config's Country Name.\n${CClear}"
        ...
        if [ $LINES -eq 0 ] #If there are no lines, error out
        then
          echo -e "\n${CRed}Error: pp.txt VPN server list is blank! Skipping SuperRandom assignment process.\n${CClear}"
      ...
      then
        ...
        while [ $svrcount -ne 60 ]
          do
            ...
            if [ $svrcount -eq 60 ]; then
              echo -e "\n${CRed}Error: Unable to reach WeVPN API! Check WeVPN service or config's Country Name.\n${CClear}"
        ...
        if [ $LINES -eq 0 ] #If there are no lines, error out
        then
          echo -e "\n${CRed}Error: wevpn.txt list is blank! Skipping SuperRandom assignment process.\n${CClear}"
      ...
      then
        ...
        while [ $loadcount -ne 60 ]
          do
            ...
            #VPNLOAD=$(curl --silent --retry 3 "https://api.nordvpn.com/v1/servers?limit=16354" | jq '.[] | select(.station == "'"$VPNIP"'") | .load') >/dev/null 2>&1
            VPNLOAD="curl --silent --retry 3 https://api.nordvpn.com/v1/servers?limit=16354 | jq '.[] | select(.station == \"$VPNIP\") | .load'"
            VPNLOAD="$(eval $VPNLOAD)"; if echo $VPNLOAD | grep -qoE '\berror.*\b'; then VPNLOAD=0; fi
      ...
      then
        ...
        while [ $loadcount -ne 60 ]
          do
            ...
            PPcurl="$(eval $PPcurl)"; if echo $PPcurl | grep -qoE '\berror.*\b'; then PPcurl=0; fi

            if [ -z $VPNLOAD ]; then break; fi
            if [ $VPNLOAD -gt 0 ]; then
