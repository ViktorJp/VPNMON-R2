...
# VPNMON-R2 v2.24b2 (VPNMON-R2.SH) is an all-in-one script that is optimized for NordVPN, SurfShark VPN and Perfect Privacy
...
# -------------------------------------------------------------------------------------------------------------------------
Version="2.24b2"                                    # Current version of VPNMON-R2
...
vpnreset() {
  ...
  # Randomly select VPN Client slots against entire field of available NordVPN server IPs for selected country
    ...
    if [ $RecommendedServer == "1" ]; then
      ...
      #curl --silent --retry 3 "https://api.nordvpn.com/v1/servers/recommendations" | jq --raw-output '.[].station' > /jffs/scripts/NordVPNRS.txt  #Extract all the closest recommended NordVPN servers to a text file
      curl --silent --retry 3 "https://api.nordvpn.com/v1/servers/recommendations?filters\[locations\]\[country\]\[name\]=="'"$NordVPNRandomCountry"'"&limit=5" | jq --raw-output '.[].station' > /jffs/scripts/NordVPNRS.txt  #Extract all the closest recommended NordVPN servers to a text file
