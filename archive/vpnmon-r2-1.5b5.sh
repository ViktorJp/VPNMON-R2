...
# VPNMON-R2 v1.5b5 (VPNMON-R2.SH) is an all-in-one simple script which compliments @JackYaz's VPNMGR program to maintain a
...
# -------------------------------------------------------------------------------------------------------------------------
Version="1.5b5"                                     # Current version of VPNMON-R2
...
checkvpn() {
  ...
  then
        while [ $CNT -lt $TRIES ]; do
          ...
          RC=$?
          ICANHAZIP=$(curl --silent --fail --interface $TUN --request GET --url http://icanhazip.com)
          IC=$?
          if [ $RC -eq 0 ] && [ $IC -eq 0 ];then
            ...
            if [ $VPNIP == "Unassigned" ];then
              ...
              #if $VPNCITY | grep -qE '\b(.*error.*|Undefined)\b' 2>/dev/null; then VPNCITY="$ICANHAZIP"; else VPNCITY="$(eval $VPNCITY)"; fi  # Huge thanks to @SomeWhereOverTheRainBow for this great grep code!
              if $VPNCITY | grep -qE '\b(error|Undefined)\b' 2>/dev/null; then VPNCITY="$ICANHAZIP"; else VPNCITY="$(eval $VPNCITY)"; fi  # Huge thanks to @SomeWhereOverTheRainBow/@iTyPsIDg for this great grep code!
