...
# VPNMON-R2 v1.2 (VPNMON-R2.SH) is an all-in-one simple script which compliments @JackYaz's VPNMGR program to maintain a
...
# -------------------------------------------------------------------------------------------------------------------------
Version="1.2"                                       # Current version of VPNMON-R2
...
updatecheck () {
  ...
  # Download the latest version file from the online repository
  ...
  if [ -f $DLVERPATH ]
    then
      ...
      if [ "$DLVersion" != "$Version" ]; then
          UpdateNotify="${CWhite}${InvRed}Update available ---------------------------------> v$DLVersion${CClear}"
          echo -e "$(date) - VPNMON-R2 - A new update is available to VPNMON-R2 v$DLVersion" >> $LOGFILE
        else
          UpdateNotify=0
      fi
  fi
...
vpnreset() {
  ...
  # Start the selected VPN Client
    ...
    #Check for any version updates from the online repository
    updatecheck
...
# -------------------------------------------------------------------------------------------------------------------------
  ...
  if [ "$1" == "-update" ]
    then
      # Check for the latest version from online repository
      updatecheck
      clear
      logo
...
while true; do
  ...
  # Display update notification if an update becomes available through online repository

  if [[ "$UpdateNotify" != "0" ]]; then
    echo -e "${CYellow}NOTICE: "$UpdateNotify
    echo -e "${CGreen}-----------------------------------------------------------------${CClear}"
  fi
