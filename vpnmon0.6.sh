...
#VPNMON 0.6 (VPNMON.SH) is a simple script that accompanies my VPNON.SH script, which compliments @JackYaz's VPNMGR 
  ...
  #If STATUS remains 0 then reset the VPN
	if [ $STATUS -eq 0 ]; then
		...
		echo -e "\n${CCyan}VPNMON is letting the VPN settle for 30 seconds${CClear}\n"
                sleep 30
		echo -e "\nResuming VPNMON in T minus $INTERVAL"
