...
# VPNMON-R2 v1.7b2 (VPNMON-R2.SH) is an all-in-one shell script which compliments @JackYaz's VPNMGR program to maintain a
...
# -------------------------------------------------------------------------------------------------------------------------
Version="1.7b2"                                      # Current version of VPNMON-R2
...
checkwan () {
  ...
  while true; do
    ...
    if [ "$(nvram get wan0_state_t)" -ne 2 ] && [ "$(nvram get wan1_state_t)" -ne 2 ] || [ $wandownbreakertrip == "1" ]
      then
...
vpnreset() {
  ...
  # Clean up API SurfShark Server Extracts
    ...
    then
      rm /jffs/scripts/surfshark.txt  #Cleanup
    fi

  # Clean up API Perfect Privacy Server Extracts
    if [ -f /jffs/scripts/pp.txt ]
    then
      rm /jffs/scripts/pp.txt  #Cleanup
    fi
...
wancheck() {
  ...
  if [ "$(nvram get wan"$WANIF"_state_t)" -eq 2 ]
    then
      ...
      if [ $WANIP == "Unassigned" ];then
        ...
        #WANCITY="$(eval $WANCITY)"; if echo $WANCITY | grep -qE '\b(.*'error'.*:.*True.*|Undefined)\b'; then WANCITY="$WANIP"; fi
...
# -------------------------------------------------------------------------------------------------------------------------
  ...
  if [ "$1" == "-h" ] || [ "$1" == "-help" ] || [ "$1" == "-config" ] || [ "$1" == "-monitor" ] || [ "$1" == "-log" ] || [ "$1" == "-update" ] || [ "$1" == "-uninstall" ] || [ "$1" == "-screen" ]
    then
      clear
    else
      clear
  ...
  if [ "$1" == "-config" ]
    ...
    if [ -f $CFGPATH ] #Making sure file exists before proceeding
      ...
      if [ "$UseSurfShark" == "0" ] && [ "$UseNordVPN" == "0" ]; then
        ...
        echo ""
        echo -e "${CCyan}11. Would you like to sync the active VPN slot with YazFi?"
        echo -e "${CYellow}(Default = No)${CClear}"
        if promptyNo; then
          SyncYazFi=1
        else
  ...
  # Testing to see if VPNON is currently running, and if so, hold off until it finishes
    while test -f "$LOCKFILE"; do
      # clear screen
        ...
        echo ""
        echo -e "${CRed}VPNON is currently performing a scheduled reset of the VPN."
        echo -e "${CRed}Retrying for normal operations every $SPIN seconds...${CClear}\n"
        echo -e "$(date +%s)" > $RSTFILE
        START=$(cat $RSTFILE)
        spinner
