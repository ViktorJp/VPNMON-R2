...
#VPNMON-R2 v0.9 (VPNMON-R2.SH) is an all-in-one simple script which compliments @JackYaz's VPNMGR program to maintain a
...
# -----------------------------------------------------------------------------------------------------------
Version="0.9"                           # Current version of VPNMON-R2
...
while true; do
  ...
  if [[ $UpdateSkynet -eq 1 ]]
    then
      ...
      echo -e "${CYellow}Ping Lo:${CWhite}${InvGreen}$PINGLOW${CClear}${CYellow} Hi:${CWhite}${InvRed}$PINGHIGH${CClear}${CYellow} ms | Skynet: ${InvBlue}[N]${CClear}${CYellow} | Config: ${InvBlue}$RANDOMMETHOD${CClear}"
  fi
