...
#VPNMON-R2 v1.0 (VPNMON-R2.SH) is an all-in-one simple script which compliments @JackYaz's VPNMGR program to maintain a
...
INTERVAL=60                             # How often it should check your VPN connection - default = 60 seconds
...
UseNordVPN=1                            # This variable assigns NordVPN as your default VPN provider.  Setting this to 0
...
NordVPNLoadReset=50                     # This variable is used to determine if the VPN needs to be reset based on if the
                                        # NordVPN server load exceeds this # (in %). Default = 50
...
# -----------------------------------------------------------------------------------------------------------
Version="1.0"                           # Current version of VPNMON-R2
...
vpnreset() {
  ...
  # Randomly select VPN Client slots against entire field of available NordVPN server IPs for selected country
    ...
    then
      ...
      then
        ...
        if [ $LINES -eq 0 ] #If there are no lines, error out
        then
          echo -e "\n${CCyan}Step 3A - Error: NordVPN.txt list is blank! Check NordVPN service or config's Country Name.\n${CClear}"
          echo -e "$(date) - VPNMON-R2 - *** ERROR: NordVPN.txt list is blank!" >> $LOGFILE
          break
        fi

        echo -e "\n${CCyan}Step 3A - Updating VPN Slots 1 - $N from $LINES SuperRandom NordVPN Server IPs\n${CClear}"
      ...
      echo -e "\n${CCyan}Step 3A - Skipping update of SuperRandom NordVPN Server IPs\n${CClear}"
  ...
  # Call VPNMGR functions to refresh server lists and save their results to the VPN client configs
    ...
    then
          echo -e "${CCyan}Step 3B - Refresh VPNMGRs NordVPN/PIA/WeVPN Server Locations and Hostnames\n${CClear}"
          ...
          echo -e "\n${CCyan}Step 3B - Skipping VPNMGR update for NordVPN/PIA/WeVPN Server Locations and Hostname\n${CClear}"
    fi
