...
# VPNMON-R2 v1.7b4 (VPNMON-R2.SH) is an all-in-one shell script which compliments @JackYaz's VPNMGR program to maintain a
...
# -------------------------------------------------------------------------------------------------------------------------
Version="1.7b4"                                     # Current version of VPNMON-R2
...
get_wan_setting() {
  ...
  if [ "$(timeout 10 nvram get wans_mode)" = "lb" ] ; then
      for prefix in $prefixes; do
          state="$(timeout 10 nvram get "${prefix}"state_t)"
          sbstate="$(timeout 10 nvram get "${prefix}"sbstate_t)"
          auxstate="$(timeout 10 nvram get "${prefix}"auxstate_t)"
          ...
          # get_wan_ifname()
          proto="$(timeout 10 nvram get "${prefix}"proto)"
          if [ "${proto}" = "pppoe" ] || [ "${proto}" = "pptp" ] || [ "${proto}" = "l2tp" ] ; then
              varval="$(timeout 10 nvram get "${prefix}"pppoe_"${varname}")"
          else
              varval="$(timeout 10 nvram get "${prefix}""${varname}")"
          fi
      done
  else
      for prefix in $prefixes; do
          primary="$(timeout 10 nvram get "${prefix}"primary)"
          [ "${primary}" = "1" ] && break
      done
...
checkwan () {
  ...
  while true; do
    ...
    if [ "$(timeout 10 nvram get wan0_state_t)" -ne 2 ] && [ "$(timeout 10 nvram get wan1_state_t)" -ne 2 ] || [ $wandownbreakertrip == "1" ]
      then
        ...
        while [ $wandownbreakertrip == "1" ]; do
          ...
          if [ "$(timeout 10 nvram get wan0_state_t)" -ne 2 ] && [ "$(timeout 10 nvram get wan1_state_t)" -ne 2 ]
            then
        ...
        if timeout 60 nc -w1 $testssl 443 && echo |openssl s_client -connect $testssl:443 2>&1 |awk 'handshake && $1 == "Verification" { if ($2=="OK") exit; exit 1 } $1 $2 == "SSLhandshake" { handshake = 1 }'
          then
    ...
    while [ $CNT -lt $TRIES ]; do # Loop through number of tries
      ...
      if [ $RC -eq 0 ] && [ $IC -eq 0 ];then  # If both ping/curl come back successful, then proceed
        ...
        if [ $VPNIP == "Unassigned" ];then # The first time through, use API lookup to get exit VPN city and display
          VPNIP=$(timeout 10 nvram get vpn_client$1_addr)
...
wancheck() {
  ...
  if [ "$(timeout 10 nvram get wan"$WANIF"_state_t)" -eq 2 ]
    then
  ...
  if [ "$1" == "-config" ]
    ...
    if [ -f $CFGPATH ] #Making sure file exists before proceeding
      then
        ...
        if [ "$UseNordVPN" == "0" ]; then
          echo ""
          echo -e "${CCyan}5-1. Is SurfShark your default VPN Provider? ${CYellow}(Default = No)${CClear}"
          if promptyNo; then
            UseSurfShark=1
          else
            UseSurfShark=0
          ...
          if [ "$UseSurfShark" == "1" ]; then
            echo ""
            echo -e "${CCyan}5-1a. Would you like to use the SurfShark SuperRandom functionality?"
            echo -e "${CYellow}(Default = No)${CClear}"
            if promptyNo; then
            ...
            echo ""
            echo -e "${CCyan}5-1b. What Country is your country of origin for SurfShark? ${CYellow}(Default = "
            echo -e "${CYellow}United States). NOTE: Country names must be spelled correctly as below!"
            ...
            if [ "$SurfSharkSuperRandom" == "1" ]; then
              echo ""
              echo -e "${CCyan}5-1c. Would you like to randomize connections across multiple countries?"
              echo -e "${CCyan}NOTE: A maximum of 2 additional country names can be added. (Total of 3)"
              echo -e "${CYellow}(Default = No)${CClear}"
              if promptyNo; then
            ...
            echo ""
            echo -e "${CCyan}5-1d. At what VPN server load would you like to reconnect to a different"
      ...
      if [ "$UseSurfShark" == "0" ] && [ "$UseNordVPN" == "0" ]; then
        echo ""
        echo -e "${CCyan}5-2. Is Perfect Privacy your default VPN Provider? ${CYellow}(Default = No)${CClear}"
        if promptyNo; then
          UsePP=1
        else
          UsePP=0
        ...
        if [ "$UsePP" == "1" ]; then
          echo ""
          echo -e "${CCyan}5-2a. Would you like to use the Perfect Privacy SuperRandom functionality?"
          echo -e "${CYellow}(Default = No)${CClear}"
          if promptyNo; then
            PPSuperRandom=1
          else
          ...
          echo ""
          echo -e "${CCyan}5-2b. What Country is your country of origin for Perfect Privacy? ${CYellow}(Default = "
          echo -e "${CYellow}United States). NOTE: Country names must be spelled correctly as below!"
          ...
          if [ "$PPSuperRandom" == "1" ]; then
            echo ""
            echo -e "${CCyan}5-2c. Would you like to randomize connections across multiple countries?"
            echo -e "${CCyan}NOTE: A maximum of 2 additional country names can be added. (Total of 3)"
            echo -e "${CYellow}(Default = No)${CClear}"
            if promptyNo; then
          ...
          echo ""
          echo -e "${CCyan}5-2d. At what VPN server load would you like to reconnect to a different"
          ...
          echo ""
          echo -e "${CCyan}5-2e. Would you like to whitelist Perfect Privacy VPN servers in the Skynet"
        ...
        echo ""
        echo -e "${CCyan}9. How many seconds would you like to delay start-up of VPNMON-R2 in"
        ...
        echo ""
        echo -e "${CCyan}10. Would you like to trim your log file when your VPN connection resets?"
        echo -e "${CYellow}(Default = No)${CClear}"
        if promptyNo; then
          TRIMLOGS=1
          echo ""
          echo -e "${CCyan}10a. How large would you like your log file to grow (in # of lines)?"
        ...
        echo ""
        echo -e "${CCyan}11. Would you like to sythe active VPN slot with YazFi?"
        echo -e "${CYellow}(Default = No)${CClear}"
        if promptyNo; then
          SyncYazFi=1
        else
        ...
        if [ "$SyncYazFi" == "1" ]; then
          echo ""
          echo -e "${CCyan}11a. Please indicate which of your YazFi guest network slots you want to"
          echo -e "${CCyan}sywith the active VPN slot? ${CYellow}(All Default = No)${CClear}"
          echo ""
    ...
    if [ -f $CFGPATH ]; then
      source $CFGPATH
        ...
        if [ ! -f "/opt/bin/timeout" ]
          then
            clear
            echo -e "${CRed}Error: VPNMON-R2 is not correctly installed.  Please run 'vpnmon-r2.sh -install' to complete setup.${CClear}"
            echo -e "${CRed}Install Entware with AMTM or re-install required Entware dependencies.${CClear}"
            echo ""
            kill 0
        fi
...
while true; do
  ...
  state1=$(timeout 10 nvram get vpn_client1_state)
  state2=$(timeout 10 nvram get vpn_client2_state)
  state3=$(timeout 10 nvram get vpn_client3_state)
  state4=$(timeout 10 nvram get vpn_client4_state)
  state5=$(timeout 10 nvram get vpn_client5_state)

  # Display the VPN states along with scheduled reset time/interval seconds
  if [ $ResetOption -eq 1 ]
    then
      echo -e "${CCyan}VPN State 1:$state1 2:$state2 3:$state3 4:$state4 5:$state5${CClear}${CGreen} ------ ${CYellow}Sched Reset: ${InvBlue}$ConvDailyResetTime${CClear}${CYellow} / ${InvBlue}$INTERVAL Sec${CClear}"
    else
      echo -e "${CCyan}VPN State 1:$state1 2:$state2 3:$state3 4:$state4 5:$state5${CClear}${CGreen} --------- ${CYellow}Interval: ${InvBlue}$INTERVAL Sec${CClear}"
  fi

  # Display the WAN states along with the Public IP of your VPN connection
  wstate0=$(timeout 10 nvram get wan0_state_t)
  wstate1=$(timeout 10 nvram get wan1_state_t)
  if [ -z $ICANHAZIP ]; then ICANHAZIP="Probing"; fi
  echo -e "${CCyan}WAN State 0:$wstate0 1:$wstate1${CGreen} ---------------- ${CYellow}Public VPN IP: ${InvBlue}$ICANHAZIP${CClear}"
  ...
  # If the NordVPN/SurfShark Server load is greater than the set variable, reset the VPN and hopefully find a better server
  ...
  elif [ $SurfSharkLoadReset -le $VPNLOAD ]; then
      echo -e "\n${CRed}SurfShark Server Load is higher than $SurfSharkLoadReset %, VPNMON-R2 is executing VPN Reset${CClear}\n"
      echo -e "$(date) - VPNMON-R2 - **SurfShark Server Load is higher than $SurfSharkLoadReset%** - Executing VPN Reset" >> $LOGFILE
  elif [ $PPLoadReset -le $VPNLOAD ]; then
      echo -e "\n${CRed}Perfect Privacy Server Load is higher than $PPLoadReset %, VPNMON-R2 is executing VPN Reset${CClear}\n"
      echo -e "$(date) - VPNMON-R2 - **Perfect Privacy Server Load is higher than $PPLoadReset%** - Executing VPN Reset" >> $LOGFILE
